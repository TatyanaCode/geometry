...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("коэфициент прямых a  и b " + slopeA +" "+ slopeB);
        ...
        if (Math.tan(slopeA) != Math.tan(slopeB)) {
          double angle =  lineService.calcAngleLine(slopeA, slopeB);
            System.out.println( "угол пересекающихся = "+ angle);
        } else {
           double distance =  lineService.distanceParallelLine(p1,p2, p3,p4);
            System.out.println("растояние между паралельными = "+ distance);
        }
